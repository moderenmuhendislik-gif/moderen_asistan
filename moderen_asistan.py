...
# --- GENEL AYARLAR ---
st.set_page_config(page_title="MODEREN LAZER ERP V65", layout="wide")
...
def veritabani_hazirla():
    # Verilen Sure ve Fark_Sn eklendi
...
menu = st.sidebar.selectbox("Bölüm Seçiniz:", ["🛠️ Usta Paneli", "📦 Depo Bölümü", "📊 Analiz Bölümü"])

# --- 🛠️ USTA PANELİ ---
if menu == "🛠️ Usta Paneli":
    if st.sidebar.text_input("Giriş Şifresi:", type="password") == USTA_SIFRE:
        ...
        if b2.button("🚨 ARZA", use_container_width=True):
            ...
            if st.session_state.a_baslangic is None:
                st.session_state.a_baslangic = time.time()
            st.rerun()
        ...
        if b4.button("🔄 SIFIRLA", use_container_width=True):
            if st.checkbox("Emin misin?"):
                st.session_state.update({"k_toplam": 0, "a_toplam": 0, "k_baslangic": None, "a_baslangic": None, "ilk_start": None})
                st.rerun()
        ...
        if st.session_state.k_baslangic or st.session_state.a_baslangic:
        ...
        with st.form("kayit_formu"):
            ...
            # --- VERİLEN SÜRE GİRİŞİ GERİ EKLENDİ ---
            st.write("⏱️ **Verilen Süre**")
            v1, v2, v3 = st.columns(3)
            vs = v1.number_input("Saat", 0)
            vd = v2.number_input("Dk", 0)
            vsn = v3.number_input("Sn", 0)
            ...
            if st.form_submit_button("🚀 KAYDET VE STOKTAN DÜŞ"):
                ...
                if f_is and st.session_state.ilk_start and mevcut > 0:
                    ...
                    # Süre hesaplamaları
                    verilen_toplam_sn = (vs * 3600) + (vd * 60) + vsn
                    kesilen_toplam_sn = int(st.session_state.k_toplam)
                    fark_saniye = kesilen_toplam_sn - verilen_toplam_sn # Eksi çıkarsa erken bitirmiş, artıysa gecikmiş
                    
                    yeni_is = {
                        'Tarih': datetime.now().strftime("%d-%m-%Y"),
                        'Baslama': st.session_state.ilk_start,
                        'Bitis': datetime.now().strftime("%H:%M:%S"),
                        'Is_Adi': f_is, 
                        'Sac_Tipi': secilen_kat, 
                        'Olcu': temiz_olcu,
                        'Kesim_Suresi': hms_format(st.session_state.k_toplam),
                        'Verilen_Sure': f"{vs:02d}:{vd:02d}:{vsn:02d}", # Formata çevrildi
                        'Fark_Sn': fark_saniye, # Tabloya eklendi
                    ...
                    st_df.to_csv(STOK_FILE, index=False)
                    st.success("Kayıt Başarılı!"); time.sleep(1); st.rerun()
                elif mevcut <= 0:
                    st.error("❌ Bu ölçüden stokta kalmamış!")
                else: 
                    st.error("Lütfen sayacı başlatın ve iş adını girin!")
...
# --- 📊 ANALİZ BÖLÜMÜ ---
    ...
    st.title("📊 Üretim Geçmişi")
    
    if st.button("🧹 Tüm Kayıtları ve Tabloyu Sıfırla (Eğer tablo bozuksa buna bas)"):
        hedef_kolonlar = ['Tarih', 'Baslama', 'Bitis', 'Is_Adi', 'Sac_Tipi', 'Olcu', 'Kesim_Suresi', 'Verilen_Sure', 'Fark_Sn', 'Verim_%']
        pd.DataFrame(columns=hedef_kolonlar).to_csv(DB_FILE, index=False)
        st.success("Veritabanı tertemiz oldu! Tablo sıfırlandı.")
        st.rerun()
